...
HTML is a computer language that humans can read. In HTML, each piece of content (e.g. a paragraph) starts and ends with a tag a computer can recognise. Tags are always in elbow brackets, like this paragraph tag: `<p>`. 
...
And if I want to tell the computer what *kind* of paragraph it is, say it's a blurb, I can put it in a class like this: `<p class="blurb">`. The `<p>` tag is one of about a hundred tags in HTML, maybe ten of which you'll only ever see in a book. Class names are infinite, because we can make them up as we need them.
...
As you work with markdown, you'll get to know it really well. That can take as little as a few hours. And hopefully you'll come to love it as much as we do. To do that, you'll need to keep the [kramdown syntax reference](http://kramdown.gettalong.org/syntax.html) handy. For now, though, we'll explain the most common, important things you'll to edit most simple books.

To quickly test any basic markdown yourself, go to [http://kramdown.herokuapp.com](http://kramdown.herokuapp.com/). Put markdown in the left, and see the resulting HTML on the right.
...
You can use up to six levels of heading, from level one to level six. To make a heading, just put one or more hash signs, `#`, and a space, before the heading. Like with paragraphs, separate the heading from everything else with an empty line space.
...
In our framework we use blockquote HTML for more than just quotations, though. We also use them for figures and poetry. We'll come back to this later.
...
Endnotes in our framework appear at the end of a document (that is, a web page or a book chapter).[^1]
...
But if your book needs classes that aren't already in our framework's stylesheets, you'll need to ask a CSS-savvy designer to write you some CSS rules for your new class, and to [put these in a child stylesheet](https://github.com/electricbookworks/book-framework#stylesheets).
...
EBW-book-framework supported classes
{:.table-caption}

...
*This is a sidenote using an inline span. It's exactly aligned with its position in the flow. In the markdown, note the `{:.sidenote}` tag after the closing `*`. For this sidenote to work on screen, a browser or ereader must support CSS to float the footnote correctly, or it'll appear in the text flow. For endnote-style notes, see [chapter&nbsp;1](1.html).*{:.sidenote}
...
Best-practice poetry layout indents a poem till it's longest line is centred on the page. To achieve this, we put the poem in a blockquote before styling it.
...
Our [detailed framework instructions](http://github.com/electricbookworks/book-framework) explain more, including how to get the line indents here.

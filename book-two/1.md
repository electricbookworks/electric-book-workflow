...
In the markdown, put `- ` before each line, to make the poem a list, where each line (including each blank line, with a space or tab after the `-`) is a list item. Add `{:.verse}` at the end to tag the entire list as verse. (Technically, a poem should not be structured as a list – it's semantically frowned upon – but it makes it possible to correctly indent runover lines among other things.)

Note that if you tag the poem title as a heading, don't put in a blank line after it: being a heading gives it its blank line already.
